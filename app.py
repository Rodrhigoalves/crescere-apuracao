...
<style>
    ...
    .stButton>button { 
        background-color: #004b87; color: white; border-radius: 4px; border: none; font-weight: 500; height: 45px; transition: all 0.2s;
    }
        ...
        with st.form("form_login"):
            ...
            if st.form_submit_button("Entrar no Sistema", use_container_width=True):
                ...
                if user_data and verificar_senha(pw_input, user_data['senha_hash']):
                    ...
                    # Definição rigorosa de níveis de acesso
                    if user_data['username'].lower() == "rodrhigo": st.session_state.nivel_acesso = "SUPER_ADMIN"
                    else: st.session_state.nivel_acesso = user_data['nivel_acesso'] # CLIENT_ADMIN ou CLIENT_OPERATOR
                    st.rerun()
...
def modulo_empresas():
    ...
    with tab_cad:
        ...
        with st.form("form_empresa"):
            ...
            apelido = c_apelido.text_input("Apelido da Unidade (Ex: Filial Lisboa)", value=f.get('apelido_unidade', ''))
            ...
            if st.form_submit_button("Gravar Unidade", use_container_width=True):
                if not nome or not cnpj: st.error("Razão Social e CNPJ são obrigatórios.")
                    ...
                    try:
                        if f['id']: 
                            cursor.execute("UPDATE empresas SET nome=%s, fantasia=%s, cnpj=%s, regime=%s, tipo=%s, cnae=%s, endereco=%s, apelido_unidade=%s WHERE id=%s", 
                                           (nome, fanta, cnpj, regime, tipo, cnae, endereco, apelido, f['id']))
                        else: 
                            cursor.execute("INSERT INTO empresas (nome, fantasia, cnpj, regime, tipo, cnae, endereco, status_assinatura, apelido_unidade) VALUES (%s,%s,%s,%s,%s,%s,%s,'ATIVO',%s)", 
...
# --- 6. MÓDULO APURAÇÃO (SIMETRIA E AUDITORIA) ---
def modulo_apuracao():
    ...
    emp_sel = c_emp.selectbox("Selecione a Unidade Operacional", df_emp.apply(lambda r: f"{r['nome']} - {r['apelido_unidade'] if r['apelido_unidade'] else r['tipo']}", axis=1))
    ...
    with col_in:
        ...
        retro = c_retro.checkbox("Lançamento Retroativo (Extemporâneo)")
        comp_origem = c_origem.text_input("Mês de Origem (MM/AAAA)", disabled=not retro)
        
        if retro:
            c_nota, c_forn = st.columns([1, 2])
            num_nota = c_nota.text_input("Nº da Nota Fiscal")
            fornecedor = c_forn.text_input("Nome do Fornecedor")
        else: num_nota = fornecedor = None
        
        st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
        # Botão solto na base para alinhamento simétrico
        if st.button("Adicionar ao Rascunho", use_container_width=True):
            ...
            elif retro and (not comp_origem or not num_nota or not fornecedor): st.error("Preencha todos os dados do documento retroativo para auditoria.")
            else:
    ...
    with col_ras:
        ...
        # Altura fixa apenas para o rascunho com barra de rolagem
    ...
    if submit_export:
        ...
        try:
            ...
            if df_export.empty: st.warning("Nenhum dado encontrado para exportação nesta competência.")
                ...
                # 1. Geração XLSX (Partidas Simples, Contas Analíticas)
                linhas_excel = []
                for _, row in df_export.iterrows():
                    ...
                    # Linha PIS (11 Colunas)
                    linhas_excel.append({"Lancto Aut.": "", "Debito": row['conta_debito'], "Credito": row['conta_credito'], "Data": data_str, "Valor": row['valor_pis'], "Cod. Historico": row['codigo_historico'] or "", "Historico": f"PIS - {texto_hist}", "Ccusto Debito": "", "Ccusto Credito": "", "Nr.Documento": row['num_nota'] or row['id'], "Complemento": ""})
                    # Linha COFINS (11 Colunas)
                    linhas_excel.append({"Lancto Aut.": "", "Debito": row['conta_debito'], "Credito": row['conta_credito'], "Data": data_str, "Valor": row['valor_cofins'], "Cod. Historico": row['codigo_historico'] or "", "Historico": f"COF - {texto_hist}", "Ccusto Debito": "", "Ccusto Credito": "", "Nr.Documento": row['num_nota'] or row['id'], "Complemento": ""})
                ...
                # 2. Geração PDF Analítico
                pdf = FPDF()
                pdf.add_page()
                pdf.set_font("Arial", 'B', 12)
                pdf.cell(190, 10, "DEMONSTRATIVO DE APURAÇÃO - PIS E COFINS", ln=True, align='C')
                pdf.set_font("Arial", '', 10)
                pdf.cell(190, 6, f"Competência: {competencia}", ln=True, align='C')
                pdf.ln(5)
                
                # Listar Débitos e Créditos separadamente
                pdf.set_font("Arial", 'B', 10)
                pdf.cell(190, 8, "1. BASE DE CÁLCULO DAS RECEITAS (DÉBITOS)", ln=True)
                pdf.set_font("Arial", '', 9)
                deb_pis = deb_cof = cred_pis = cred_cof = 0
                
                for _, r in df_export[df_export['op_tipo'] == 'RECEITA'].iterrows():
                    pdf.cell(90, 6, f"{r['op_nome']} {'[EXTEMP]' if r['origem_retroativa'] else ''}", 1)
                ...
                pdf.ln(5)
                pdf.set_font("Arial", 'B', 10)
                pdf.cell(190, 8, "2. BASE DE CÁLCULO DOS INSUMOS E CRÉDITOS", ln=True)
                pdf.set_font("Arial", '', 9)
                ...
                # Quadro de Apuração e Saldo
                pdf.ln(10)
                pdf.set_font("Arial", 'B', 10)
                pdf.cell(190, 8, "3. QUADRO DE APURAÇÃO FINAL", ln=True)
                pdf.set_font("Arial", '', 10)
                
                res_pis = deb_pis - cred_pis
                res_cof = deb_cof - cred_cof
                
                pdf.cell(120, 6, "Total Imposto a Recolher (Se Débitos > Créditos):", 0)
                pdf.cell(35, 6, formatar_moeda(max(0, res_pis)), 0)
                pdf.cell(35, 6, formatar_moeda(max(0, res_cof)), 0, ln=True)
                
                pdf.cell(120, 6, "Saldo Credor para o Mês Seguinte (Se Créditos > Débitos):", 0)
                pdf.cell(35, 6, formatar_moeda(abs(min(0, res_pis))), 0)
                pdf.cell(35, 6, formatar_moeda(abs(min(0, res_cof))), 0, ln=True)

                # Busca de Lançamentos Extemporâneos para Nota de Auditoria
                ...
                if not df_notas.empty:
                    ...
                    pdf.cell(190, 6, "AVISO DE AUDITORIA: LANÇAMENTOS EXTEMPORÂNEOS", ln=True)
                    pdf.set_font("Arial", '', 8)
                    pdf.multi_cell(190, 5, "Os documentos abaixo pertencem a esta competência, mas foram rececionados com atraso e os seus créditos aproveitados em competências posteriores:")
                ...
                c_btn1.download_button("⬇️ XLSX (ERP - Partidas Simples)", data=buffer, file_name=f"LCTOS_{comp_db}.xlsx")
...
# --- 8. MÓDULO PARÂMETROS CONTÁBEIS (RESTRIÇÃO MULTI-TENANT) ---
def modulo_parametros():
    if st.session_state.nivel_acesso == "CLIENT_OPERATOR":
        st.error("Acesso restrito aos Administradores da Empresa.")
        return
    st.markdown("### ⚙️ Parâmetros Contábeis e Integração ERP")
    st.info("Configuração das contas analíticas (sem pontos) e padrões de histórico.")
    # Implementação futura do CRUD de operações contábeis aqui.
...
with st.sidebar:
    ...
    st.write("---")
    # Link externo integrado simetricamente como botão
