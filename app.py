...
if not st.session_state.autenticado:
  # --- BOTÃO DE EMERGÊNCIA (COLE EXATAMENTE ASSIM) ---
if not st.session_state.autenticado:
...
if st.button("🆘 RESETAR MINHA SENHA AGORA"):
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            # Gerando o hash via Python para evitar erro de colação do MySQL
            ...
            conn.commit()
            conn.close()
            st.success("Senha resetada com sucesso! Tente logar com: rodrhigo / Crescere@2026")
        except Exception as e:
            st.error(f"Erro ao resetar: {e}")

    # ... aqui continua o seu código da c1, login_col, c3 ...
    st.markdown("<br><br>", unsafe_allow_html=True)
