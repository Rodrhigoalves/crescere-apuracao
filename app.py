def testar_fluxo_basico():
    ...
    # Limpeza de dados de teste para permitir rodar várias vezes sem erro
    db.execute("DELETE FROM custos WHERE empresa_id = ? AND competencia = ?", (empresa_id, competencia_teste))
    db.execute("DELETE FROM lancamentos WHERE empresa_id = ? AND competencia IN (?, ?)", (empresa_id, "2026-01", competencia_teste))
    db.execute("DELETE FROM fechamentos WHERE empresa_id = ? AND competencia = ?", (empresa_id, competencia_teste))
    db.commit()
