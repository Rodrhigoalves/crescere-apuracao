...
def _is_super_admin(username: str) -> bool:
    try:
        ...
        admins = ["rodrhigo"]
    return username.lower() in [u.lower() for u in admins]
...
def carregar_empresas_visiveis(nivel_acesso: str, contabilidade_id, usuario_id, empresa_id_legacy):
    ...
    with get_db_connection() as conn:
        ...
        """
        df = pd.read_sql(query, conn, params=(int(contabilidade_id), int(usuario_id)))

        if df.empty and empresa_id_legacy:
            query_fallback = "SELECT * FROM empresas WHERE id = %s AND status_assinatura = 'ATIVO'"
            df = pd.read_sql(query_fallback, conn, params=(int(empresa_id_legacy),))
            ...
            if st.button("Consultar CNPJ", use_container_width=True):
                ...
                if res and res.get('status') != 'ERROR':
                    st.session_state.dados_form.update({
                        "nome": res.get('nome', ''), "fantasia": res.get('fantasia', ''),
                        "cnpj": res.get('cnpj', ''),
            ...
            if df_destinos_custo.empty:
                ...
                if st.button("Adicionar Custo Avulso", use_container_width=True, type="secondary"):
                    ...
                    if not comp_valida: st.error("O formato da Competência deve ser MM/AAAA válido.")
                        ...
                        dummy_op_id = int(df_op.iloc[0]['id'])
    ...
    if st.button("Gerar Ficheiros e Analisar Saldos"):
        ...
        try:
            with get_db_connection() as conn:
                ...
                """
                df_export = pd.read_sql(query, conn, params=filtro_params + [comp_db])

                query_hist = f"""
                    SELECT o.tipo as op_tipo, SUM(l.valor_pis) as t_pis, SUM(l.valor_cofins) as t_cof,
                    ...
                    WHERE {filtro_empresa} AND l.competencia < %s AND l.status_auditoria = 'ATIVO' AND l.is_custo_avulso = 0
                    GROUP BY o.tipo
                """
                df_hist = pd.read_sql(query_hist, conn, params=filtro_params + [comp_db])

                query_fut = f"""
                    SELECT * FROM lancamentos l
                    WHERE {filtro_empresa}
                      AND l.competencia_origem = %s
                      AND l.competencia != %s
                      AND l.status_auditoria = 'ATIVO'
                      AND l.is_custo_avulso = 0
                """
                df_fut = pd.read_sql(query_fut, conn, params=filtro_params + [comp_db, comp_db])
        ...
        if not df_todos.empty:
            ...
            for _, rb in df_todos.iterrows():
                ...
                if rb['status'] == 'ativo': dt_ref = hoje_br.date()
                    ...
                    if isinstance(dt_ref, (datetime, pd.Timestamp)): dt_ref = dt_ref.date()
            ...
            if c_btn.button("Gerar PDF do Inventário", type="primary", use_container_width=True):
                ...
                if df_pdf.empty:
                    ...
                    for _, r in df_pdf.iterrows():
                        ...
                        if isinstance(dt_base, (datetime, pd.Timestamp)): dt_base = dt_base.date()

                        dt_compra_orig = r['data_compra']
                        if isinstance(dt_compra_orig, (datetime, pd.Timestamp)): dt_compra_orig = dt_compra_orig.date()

                        if dt_compra_orig > data_posicao: continue

                        if r['status'] != 'ativo' and pd.notnull(r.get('data_baixa')):
                            dt_baixa = r['data_baixa']
                            if isinstance(dt_baixa, (datetime, pd.Timestamp)): dt_baixa = dt_baixa.date()
                        ...
                        if pd.notnull(r.get('data_saldo_inicial')):
                            if data_posicao < dt_base: valor_residual = base_calc; dep_acumulada = 0.0
